...
class CustomCOCOEvaluator(COCOEvaluator):
    # def __init__(self, dataset_name, cfg, *args, **kwargs):
    def __init__(self, dataset_name, cfg, distributed=True, output_dir=None):
        ...
        if not cfg.INPUT.REMAP_CATEGORY_ID:
            return
        ...
        remap_list = cfg.INPUT.REMAP_CATEGORY_MAP
        remap = {i: remap_list[i] for i in range(len(remap_list))}
        
        for ann in coco_data["annotations"]:
            ann["category_id"] = remap[ann["category_id"]]

        # create new COCO API instance
        self._coco_api = COCO()
        self._coco_api.dataset = coco_data
        self._coco_api.createIndex()

        # update ppdd_categories
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        base_names = set([x["name"] for x in ppdd_categories_base])
        novel_names = set([x["name"] for x in ppdd_categories_novel])
        # extra_names = set(
