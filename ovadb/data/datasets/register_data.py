...
def load_ppdd_json(
    ...
    cfg.merge_from_file("/home/project/host_workspace/PythonProjects/PPDD_OVCLS/ovad-benchmark-code/configs/Base_PPDD_C4_1x_train_nyem.yaml")

    if cfg.INPUT.REMAP_CATEGORY_ID:
        remap_map = cfg.INPUT.REMAP_CATEGORY_MAP
        remap_dict = {old:new for new, old in enumerate(remap_map)}    

        for ann in coco_api.dataset["annotations"]:
            old_id = ann["category_id"]
            if old_id in remap_dict:
                ann["category_id"] = remap_dict[old_id]              
...
def load_custom_json(
    ...
    # urgent fix: change category names to match PPDD OVAD
    from detectron2.config import get_cfg
    from ovadb.config import add_ovadb_config
    cfg = get_cfg()
    add_ovadb_config(cfg)
    ...
    if cfg.INPUT.REMAP_CATEGORY_ID: # [0,1,4,5,2,3]
        remap_map = cfg.INPUT.REMAP_CATEGORY_MAP
        remap_dict = {old:new for new, old in enumerate(remap_map)}    
          
        catid2contid = remap_dict
    
        # original_cats = sorted(lvis_api.dataset["categories"], key=lambda x: x["id"])
        # catid2contid = {}

        # for i, cat in enumerate(original_cats):
        #     old_id = cat["id"]
        #     if old_id in remap_dict:
        #         catid2contid[old_id] = remap_dict[old_id]

        print("##### catid2contid:", catid2contid, lvis_api.dataset["categories"]) 
    ...
    for anns_per_image in anns:
        for ann in anns_per_image:
            ...
            if old_cat_id in remap_dict:
                ann["category_id"] = remap_dict[old_cat_id] 
