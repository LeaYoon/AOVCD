...
def reset_attribute_test_model(
    ...
    module.roi_heads.box_predictor.att_cls_score.set_embeddings(
        "", is_noun=True, zs_weight=zs_weight
    )
    assert (
        module.roi_heads.box_predictor.att_cls_score.num_classes == num_classes
    ), "Number of classes loaded in weights {} must match classes in metadata {}".format(
        module.roi_heads.box_predictor.att_cls_score.num_classes, num_classes
    )
    ...
    # load attributes
    num_attributes = len(metadata.attribute_classes)
    module.roi_heads.box_predictor.att_cls_score.set_embeddings(att_weights_path, is_noun=False)
    assert (
        module.roi_heads.box_predictor.att_cls_score.num_attributes == num_attributes
    ), "Number of attributes loaded in weights {} must match classes in metadata {}".format(
        module.roi_heads.box_predictor.att_cls_score.num_attributes, num_attributes
    )
